...
from Buckets.components.indicators import EmptyIndicator  # ← add this import
from Buckets.config import CONFIG
from Buckets.forms.category_form import CategoryForm
from Buckets.managers.categories import (
    create_category,
    delete_category,
...
class Categories(Static):
    ...
    def rebuild(self) -> None:
        ...
        empty: EmptyIndicator = self.query_one(EmptyIndicator)  # ← get the indicator
        ...
        if categories:
            for category, node, depth in categories:
                ...
                table.add_row(node, char + category.name, nature, key=category.id)
    ...
    def compose(self) -> ComposeResult:
        ...
        yield EmptyIndicator("No categories")  # ← add the indicator
