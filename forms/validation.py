...
def _validate_autocomplete(
    ...
    # two autocomplete modes: with item.text, or value-only
...
def validateForm(
    ...
    Expects formData.fields list with .key, .type, .is_required, .min/.max, .options, .autocomplete_selector
    and each field widget mounted with id=f"field-{key}" and .value (or .heldValue for autocomplete).
    """
    result: Dict[str, Any] = {}
    errors: Dict[str, str] = {}
    isValid = True
            ...
            case "autocomplete":
                if getattr(field, "autocomplete_selector", False):
                    ok, error = _validate_autocomplete(
                        getattr(fieldWidget, "value", ""), fieldValue, field
                    )
                    if ok and fieldValue:
                        result[key] = fieldValue
                else:
                    if not getattr(fieldWidget, "value", "") and getattr(
                        field, "is_required", False
                    ):
                        error = "Required"
                    else:
                        result[key] = getattr(fieldWidget, "value", "")
            case _:
